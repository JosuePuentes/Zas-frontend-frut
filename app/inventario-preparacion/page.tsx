...
export default function InventarioPreparacionPage() {
  ...
  useEffect(() => {
    api
      .getAvailability()
      .then((d) => setBatidos(Array.isArray(d) ? d : []))
      .catch(() => setBatidos([]))
  ...
  return (
    <div className="space-y-6">
          ...
          {batidos.map((item, i) => (
            ...
            >
              ...
              <p className="text-white/90 text-sm mt-1">
                Stock dosis: {item.stock_dosis ?? 0}
              </p>
              <p className="text-white/90 text-sm">
                Producibles: {item.producibles_con_materia_prima ?? 0}
              </p>
              <p className="text-white/90 text-sm font-medium">
                Total disponible: {item.total_disponible ?? 0}
              </p>
              <p className="text-white/80 text-sm mt-2">${item.precio_sugerido?.toFixed(2)}</p>
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
      ...
      {batidos.length === 0 && (
        ...
        >
          <Utensils className="w-16 h-16 mx-auto mb-4 text-frutal-kiwi/50" />
